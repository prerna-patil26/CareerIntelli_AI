...
class QuestionSelector:
    def __init__(self, df):
        ...
        if 'role' not in self.df.columns:
            raise Exception("❌ 'role' column not found in dataset")

        if 'question_type' not in self.df.columns:
            raise Exception("❌ 'question_type' column not found")
    ...
    def get_available_domains(self):
        domains = self.df['role'].dropna().unique().tolist()
    ...
    def select_questions(self, domain, total_questions=15, hr_count=4):
        ...
        hr_df = role_df[role_df['question_type'] == 'hr']
        tech_df = role_df[role_df['question_type'] == 'technical']
